try {
         ...
         Properties.OnPlayerProperty.Add (function (context, e) 
         {
             ...
             p.Team.Properties.Get(p.Id + 'info1').Value = '<color=#FFFFFF>  Звание: ' + (p.Properties.Get('rank').Value = Prop.Get(p.Id + 'rank').Value ? p.Properties.Get('rank').Value = Prop.Get(p.Id + 'rank').Value : Prop.Get(p.Id + 'rank').Value = p.Properties.Get('rank').Value) + '  ' + n + '' + n + '   level: ' + (p.Properties.Get('level').Value = Prop.Get(p.Id + 'level').Value ? p.Properties.Get('level').Value = Prop.Get(p.Id + 'level').Value : Prop.Get(p.Id + 'level').Value += p.Properties.Get('level').Value) + ', exp: ' + (p.Properties.Get('experience').Value = Prop.Get(p.Id + 'experience').Value ? p.Properties.Get('experience').Value = Prop.Get(p.Id + 'experience').Value : Prop.Get(p.Id + 'experience').Value += p.Properties.Get('experience').Value) + ' <size=58.5>/ ' + (p.Properties.Get('next').Value = Prop.Get(p.Id + 'next').Value ? p.Properties.Get('next').Value = Prop.Get(p.Id + 'next').Value : Prop.Get(p.Id + 'next').Value += p.Properties.Get('next').Value) + '</size></color>  ';
