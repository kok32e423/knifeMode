try {
         ...
         Properties.OnPlayerProperty.Add (function (context, e) 
         {
             ...
             p.Team.Properties.Get(p.Id + 'info1').Value = '<color=#FFFFFF>  Звание: ' + (Props.Get(p.Id + 'rank').Value = p.Properties.Get('rank').Value || RANKS[0].name)  + '  ' + n + '' + n + '   level: ' + (Props.Get(p.Id + 'level').Value = p.Properties.Get('level').Value || 1)  + ', exp: ' + (Props.Get(p.Id + 'experience').Value = p.Properties.Get('experience').Value || 0) + ' <size=58.5>/ ' +  (Props.Get(p.Id + 'next').Value = p.Properties.Get('next').Value || 25) + '</size></color>  ';            
