try {
         ...
         Properties.OnPlayerProperty.Add (function (context, e) 
         {
             ...
             if (e.Name === 'experience' && e.Value >= p.Properties.Get('next').Value) return p.Properties.Get('level').Value += 1, p.Properties.Get('next').Value = RANKS[p.Properties.Get('level').Value - 1].exp, p.Properties.Get('rank').Value = RANKS[p.Properties.Get('level').Value - 1].name;    
         });
